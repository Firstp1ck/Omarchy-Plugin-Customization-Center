...
def test_themes_page_and_resolver_with_qmltestrunner():
    ...
    completed = subprocess.run(
        [runner, "-input", str(test_file), "-import", str(TEST_IMPORTS), "-import", str(ROOT)],
        cwd=ROOT, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=90,
        check=False,
    )
