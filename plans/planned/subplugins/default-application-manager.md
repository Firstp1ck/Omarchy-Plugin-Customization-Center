...
8. "Stop tracking" calls `BackendClient.abandon(transactionId)`, which runs `ccctl abandon <transactionId>`. The journal rolls back with reason `user`. The handoff itself is non-reversible and may continue, but every completed setter before it runs its inverse. The page states that stopping tracking does not cancel the external terminal, then allows new install plans once rollback settles.
